...
;(function(){
    ...
    Album.prototype = {
        ...
        ctor:function(){
            this.el   = P.dom.create('div',null,'p-ctrl-album',null);
            ...
            this.group = P.dom.create('div',null,null,'position:absolute;left:0;width:100%;top:0;height:24px;line-height:24px;color:#fff;',this.el);
            this.expand = P.dom.create("div",null,'p-expand',"position:absolute;top:-32px;right:0;height:32px;width:32px;background-color:rgba(0,0,0,0.5);",this.el);
            this.pano.addEventListener("pano_changed" , this._fetch , this);
        ...
        _fetch : function(){
            ...
            var group = {};
            //相同分组
            if(this.data && this.data.panos && this.data.panos[id]) {
                this.tabIndex = this.data.panos[id].group_id;
                local.drawGroup();
                return;
            };

            $.getJSON(P.parse(this.options.api,{id:id}),function(resp){
                var data = resp.data;
                var panos = {};
                ...
                local.tabIndex = data.length ? data[0].id : undefined;
                local.data = {
                    "raw" : data,
                    "group":group,
                    "panos":panos
    ...
    P.Control.Album = Album;
    P.control.album = function(opts){
...
;(function(){
    var creater = {
        "hot":function(d){
            var html = '<form><input type="hidden" name="ID" value="'+ d.id+'"/><input type="hidden" name="PANO_ID" value="'+ d.pano_id +'"/><input type="hidden" name="PANO_KEY" value="'+ d.pano_key +'"/><ul><li><span>名称</span><input type="text" name="TITLE" value="'+ d.title+'"/></li><li><span>经度</span><input type="text" name="LON" id="p_lng" value="'+ d.lon+'"/></li><li><span>纬度</span><input id="p_lat" type="text" name="LAT" value="'+ d.lat+'"/></li><li><span>高度</span><input type="text" name="HEIGHT" value="'+ d.height+'"/></li><li><span>描述</span><textarea type="text" name="CONTENT">'+ d.content +'</textarea></li><li><span>响应</span><select name="TYPE"><option value="0">无操作</option><option value="1">跳转</option><option value="2">播放音乐</option><option value="3">全景</option><option value="4">弹出介绍框</option></select></li><li><span>值</span><input type="text" name="TYPE_VALUE" value="'+ d.type_value+'"/></li></ul><button class="save" type="button">保存</button><button type="button" class="delete">删除</button></form><div class="close" type="button">x</div>';
            return html;
        },
        ...
        'list':function(d){
            ...
            for(var i in d){
                ...
                html += '<li data-id="'+d[i].id+'">'+d[i].title+'<span><a data-id="'+d[i].id+'" data-status="'+code+'" class="pub pub-'+d[i].status+'">'+status+'</a></span></li>';
            }
            return '<ul>'+html+'</ul>'
        }
    }
    ...
    editerControl.prototype = {
        ...
        _initEvents:function(){
            ...
            el.on('click','a.pub:not(.dis)',function(e){
    ...
    P.extend(editerControl.prototype , {
        ...
        _onSave : function(btn){
            ...
            this.save(data , this.action || 'GET' , function(resp){
                if(resp.status){
                    //更新信息
    ...
    P.control.editer = function(opts){
